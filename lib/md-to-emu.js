 ...
 * that does not change when the heading is reworded upstream. The other
 * clauses keep their generated IDs.
 */
export async function markdownToEmuClauses(source, idPrefix, { rootId } = {}) {
  ...
  for (const s of sections) {
    closeTo(s.level);
    counter++;
    const id = rootId && counter === 1 && s.level === 1
      ? rootId
      : slugify(s.title, `${idPrefix}-${counter}`);
