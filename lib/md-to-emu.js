 ...
 *
 * `normativeOptional` marks the document's top clause with ecmarkup's
 * `normative-optional` attribute: its requirements apply only to
 * implementations that provide the feature it describes.
 *
 ...
 * inside annexes.
 */
export async function markdownToEmuClauses(source, idPrefix, { rootId, element = "emu-clause", normativeOptional = false } = {}) {
  ...
  for (const s of sections) {
    ...
    const optional = normativeOptional && counter === 1 ? " normative-optional" : "";
    html += `<${element} id="${escapeAttr(id)}"${oldids}${optional}>\n`;
